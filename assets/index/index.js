function initIndexPage() {
  ...
  function updateBackgrounds() {
    if (!document.getElementById('astronautImage')) return;
  ...
  (function () {
    ...
    function init() {
      const username = window.githubUsername || 'Souravrajvi0';
    ...
    if (window.githubUsername) {
      ...
      document.addEventListener('configLoaded', function (event) {
        ...
        const username = config.githubStats ? config.githubStats.username : 'Souravrajvi0';
        fetchLiveGitHubStats(username);
      });
    }
  })();
