function initAboutPage() {
    ...
    function updateButtons() {
        ...
        switch (currentSection) {
            case 'main':
                ...
                buttons.bottom.textContent = 'PRs Merged';
                break;
            case 'left':
                buttons.right.classList.remove('hidden');
    ...
    function loadPRPage(page) {
        ...
        const startIndex = (page - 1) * prPerPage;
        const pageItems = prList.slice(startIndex, startIndex + prPerPage);
        ...
        pageItems.forEach((pr) => {
            ...
            card.style.cursor = `url('assets/index/images/pixelboyhead.png') 12 12, pointer`;
            card.innerHTML = `
                <p class="pr-card-repo">${pr.repo}${pr.number ? ` #${pr.number}` : ''}</p>
                <p class="pr-card-title">${pr.title}</p>
                ${pr.note ? `<p class="pr-card-note">${pr.note}</p>` : ''}
                <span class="pr-card-badge">MERGED</span>
            `;
        ...
        document.getElementById('next-page').disabled = currentPage === Math.ceil(prList.length / prPerPage);
    }
    ...
    document.getElementById('next-page').addEventListener('click', () => {
        if (currentPage < Math.ceil(prList.length / prPerPage)) {
            currentPage++;
            loadPRPage(currentPage);
        }
    });
